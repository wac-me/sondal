...
const theme = {
  ...
  surfaceHigh: "#232630",
  accent: "#3B7DD8",
  accentDim: "rgba(59,125,216,0.12)",
  accentBright: "#5B9AEA",
  indigo: "#2D3561",
  ...
  textDim: "#4A5070",
  border: "#2A2D3A",
  borderAccent: "rgba(59,125,216,0.3)",
  red: "#E8373D",
  green: "#22C55E",
};

const heroSlides = [
  { id:1, tag:"GUS • GOSPODARKA", date:"Czerwiec 2025", title:"Zarobki w Polsce rosły o 8,4% r/r. Czy Twoja pensja też poszła w górę?", chartData:[{label:"2022",value:55},{label:"2023",value:72},{label:"2024",value:84},{label:"2025",value:100}], options:["Tak, zarabiam więcej","Nie — bez zmian lub mniej"], votes:6821, comments:214, split:[38,62] },
  { id:2, tag:"BANK ŚWIATOWY • MIESZKALNICTWO", date:"Maj 2025", title:"Polska w czołówce UE pod względem wzrostu cen mieszkań. Czy to kryzys?", chartData:[{label:"2021",value:40},{label:"2022",value:58},{label:"2023",value:79},{label:"2024",value:100}], options:["Tak, to prawdziwy kryzys","Nie, rynek się stabilizuje"], votes:4302, comments:189, split:[71,29] },
  { id:3, tag:"EUROSTAT • INFLACJA", date:"Czerwiec 2025", title:"Inflacja w strefie euro spada. Polacy jednak wciąż odczuwają drożyznę?", chartData:[{label:"2022",value:100},{label:"2023",value:72},{label:"2024",value:51},{label:"2025",value:38}], options:["Tak, nadal płacę dużo więcej","Nie, ceny wróciły do normy"], votes:3917, comments:97, split:[82,18] },
  { id:4, tag:"MEN • EDUKACJA", date:"Maj 2025", title:"Reforma programu nauczania 2025. Czy szkoła uczy tego, co naprawdę potrzebne?", chartData:[{label:"Mat.",value:78},{label:"J.Ang.",value:91},{label:"Prog.",value:34},{label:"Fin.",value:18}], options:["Tak, program jest wystarczający","Nie, szkoła jest przestarzała"], votes:8441, comments:312, split:[19,81] },
  { id:5, tag:"GUS • DEMOGRAFIA", date:"Kwiecień 2025", title:"Polska traci 100 000 mieszkańców rocznie. Czy polityka prorodzinna działa?", chartData:[{label:"2021",value:62},{label:"2022",value:55},{label:"2023",value:48},{label:"2024",value:40}], options:["Tak, widzę poprawę","Nie, to nie wystarczy"], votes:5590, comments:228, split:[24,76] },
];
...
const communityPolls = [
  { id:20, user:"@ania_wawa", avatar:"A", time:"8 min temu", tag:"#lokalne", question:"Czy rondo przy Dworcu Centralnym powinno mieć naziemne przejścia dla pieszych?", options:["Tak — ułatwi życie pieszym","Nie — sparaliżuje ruch aut"], baseSplit:[64,36], totalVotes:2341 },
  { id:21, user:"@marek_gdansk", avatar:"M", time:"22 min temu", tag:"#gospodarka", question:"Czy planujesz zakup własnego mieszkania w ciągu najbliższych 3 lat?", options:["Tak, to mój cel","Nie, stać mnie tylko na wynajem","Jeszcze nie wiem"], baseSplit:[28,51,21], totalVotes:4102 },
  { id:22, user:"@zuzanna.k", avatar:"Z", time:"1 godz. temu", tag:"#rozrywka", question:"Który serwis streamingowy jest według Ciebie najlepszy w 2025?", options:["Netflix","Max (HBO)","Disney+","Polsat Box Go"], baseSplit:[44,29,15,12], totalVotes:8934 },
];

const officialStats = [
  { label:"Bezrobocie PL", value:"4,9%", delta:"−0.3pp", positive:true, src:"GUS • Maj 2025" },
  { label:"Inflacja CPI", value:"3,2%", delta:"−1.1pp", positive:true, src:"NBP • Czerwiec 2025" },
  { label:"PKB r/r", value:"+3,8%", delta:"+0.6pp", positive:true, src:"GUS • Q1 2025" },
  { label:"Stopa ref. NBP", value:"5,75%", delta:"—", positive:null, src:"NBP • Czerwiec 2025" },
];
...
function LogoMark({ size = 20 }) {
  const s = size;
  return (
    <svg width={s * 0.55} height={s * 0.65} viewBox="0 0 22 28" fill="none">
...
function Tag({ children }) {
  return <span style={{ background:theme.accentDim, color:theme.accentBright, fontSize:10, fontWeight:700, padding:"3px 8px", borderRadius:4, fontFamily:"Inter, sans-serif", letterSpacing:"0.05em", whiteSpace:"nowrap" }}>{children}</span>;
}
      ...
      {data.map((d,i) => (
        <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3, height:"100%" }}>
          <div style={{ flex:1, width:"100%", display:"flex", alignItems:"flex-end" }}>
            <div style={{ width:"100%", height:`${(d.value/max)*100}%`, background: i===data.length-1 ? theme.accent : `${theme.accent}44`, borderRadius:"3px 3px 0 0", minHeight:4 }} />
          </div>
...
function VoteButtons({ options, split, voted, onVote }) {
  if (voted === null) return (
    <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
      {options.map((opt,i) => (
        <button key={i} onClick={() => onVote(i)}
          style={{ background:theme.surfaceHigh, border:`1px solid ${theme.border}`, borderRadius:9, padding:"11px 14px", color:theme.text, fontFamily:"Inter, sans-serif", fontSize:13, textAlign:"left", cursor:"pointer", transition:"border-color 0.15s" }}
...
function StickyHeader({ nowActive }) {
  return (
    <div style={{ position:"sticky", top:0, zIndex:200, background:`${theme.bg}F2`, backdropFilter:"blur(12px)", borderBottom:`1px solid ${theme.border}`, padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        ...
        <div>
          <div style={{ display:"flex", alignItems:"baseline", gap:2 }}>
            ...
            <span style={{ fontFamily:"Inter, sans-serif", fontWeight:500, fontSize:12, color:theme.accent, letterSpacing:"0.01em" }}>.top</span>
          </div>
      ...
      {nowActive && (
        <div style={{ display:"flex", alignItems:"center", gap:5, background:`${theme.red}14`, border:`1px solid ${theme.red}35`, borderRadius:20, padding:"4px 10px" }}>
          <PulsingDot />
...
function HeroSlider() {
  ...
  const resetTimer = () => {
    ...
    timerRef.current = setInterval(() => {
      setAnimating(true);
      setTimeout(() => { setCurrent(c => (c+1) % heroSlides.length); setAnimating(false); }, 200);
    }, 6000);
  };
  ...
  const goTo = (i) => {
    if (i === current) return;
    setAnimating(true);
    setTimeout(() => { setCurrent(i); setAnimating(false); }, 180);
    resetTimer();
  };
  ...
  return (
    <div style={{ background:theme.surface, borderBottom:`1px solid ${theme.border}`, padding:"16px 16px 14px" }}>
      ...
      <div style={{ opacity: animating ? 0 : 1, transition:"opacity 0.18s ease" }}>
        ...
        <div style={{ marginTop:12, borderTop:`1px solid ${theme.border}`, paddingTop:12 }}>
          <p style={{ color:theme.accent, fontFamily:"Inter, sans-serif", fontSize:10, fontWeight:600, margin:"0 0 8px", letterSpacing:"0.05em", textTransform:"uppercase" }}>A jak jest u Ciebie?</p>
          <VoteButtons options={slide.options} split={slide.split} voted={voted[slide.id]??null} onVote={i => setVoted(v => ({...v,[slide.id]:i}))} />
        </div>
      </div>
        ...
        {officialStats.map((s,i) => (
          <div key={i} style={{ background:theme.surface, border:`1px solid ${theme.border}`, borderRadius:10, padding:"10px 12px", flexShrink:0, minWidth:108 }}>
            ...
            <div style={{ display:"flex", alignItems:"baseline", gap:5 }}>
              ...
              <span style={{ color: s.positive===true ? theme.green : s.positive===false ? theme.red : theme.textDim, fontSize:10, fontFamily:"Inter, sans-serif", fontWeight:600 }}>{s.delta}</span>
            </div>
...
function CreatorBanner({ onGoToCreate }) {
  return (
    <div style={{ background:`linear-gradient(135deg, ${theme.accentDim}, ${theme.indigoDim})`, border:`1px solid ${theme.borderAccent}`, borderRadius:14, padding:"16px", margin:"0 0 12px" }}>
      <p style={{ color:theme.text, fontFamily:"'Plus Jakarta Sans', sans-serif", fontSize:15, fontWeight:700, margin:"0 0 6px" }}>Masz pytanie do świata? 🌍</p>
      <p style={{ color:theme.textMuted, fontFamily:"Inter, sans-serif", fontSize:12, margin:"0 0 12px", lineHeight:1.5 }}>Stwórz własną sondę w 10 sekund. Zbierz głosy, osadź wykres na swojej stronie.</p>
      <button onClick={onGoToCreate} style={{ background:theme.accent, color:"#fff", border:"none", borderRadius:8, padding:"10px 16px", fontFamily:"'Plus Jakarta Sans', sans-serif", fontSize:13, fontWeight:700, cursor:"pointer" }}>Przejdź do kreatora →</button>
    </div>
  );
}
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes pulse { 0%{box-shadow:0 0 0 0 rgba(232,55,61,0.7)} 70%{box-shadow:0 0 0 6px rgba(232,55,61,0)} 100%{box-shadow:0 0 0 0 rgba(232,55,61,0)} }
